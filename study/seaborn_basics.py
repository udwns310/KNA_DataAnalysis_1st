...
# [문제 상황] 숫자 비교엔 약하고 모양·색 차이엔 강한 사람 눈을 위한 도구
...
plt.title("소입로온도 분포")
plt.xlabel("소입로온도(℃)")
plt.savefig(os.path.join("data", "_tmp_sns_practice1.png"))
plt.close()
...
plt.savefig(os.path.join("data", "_tmp_sns_practice2.png"))
plt.close()
...
plt.savefig(os.path.join("data", "_tmp_sns_practice3.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join("data", "_tmp_sns_practice4.png"))
plt.close()
...
plt.ylabel("소입로온도(℃)")
plt.savefig(os.path.join("data", "_tmp_sns_box1.png"))
plt.close()
...
plt.savefig(os.path.join("data", "_tmp_sns_box2.png"))
plt.close()

sns.boxplot(data=df, x="라인", y="소입로온도", hue="판정")
plt.title("라인별 · 판정별 소입로온도 분포")
plt.savefig(os.path.join("data", "_tmp_sns_box3.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join("data", "_tmp_sns_bar1.png"))
plt.close()
...
plt.title("라인별 판정 개수")
plt.savefig(os.path.join("data", "_tmp_sns_bar2.png"))
plt.close()
...
# [문제 상황] 단변량에서 이변량으로 - 두 변수가 함께 어떻게 움직이는가
...
plt.savefig(os.path.join("data", "_tmp_sns_scatter1.png"))
plt.close()
...
plt.savefig(os.path.join("data", "_tmp_sns_scatter2.png"))
plt.close()
...
plt.savefig(os.path.join("data", "_tmp_sns_heatmap1.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join("data", "_tmp_sns_report1.png"))
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join("data", "_tmp_sns_report2.png"))
plt.close()


print("\n===================== 실습 6. 시각화 리포트 통합·저장 =====================")
# 목표: 핵심 그래프 네 개를 2x2로 모아 리포트 파일로 저장(show보다 항상 savefig 먼저)
# 단계: ① subplots로 2x2 격자 만들기 -> ② 분포·박스플롯·산점도·heatmap을 네 칸에 배치
#      -> ③ savefig로 저장하고 파일 존재 확인
# 예상 결과: 분포·박스·관계·상관을 담은 2x2 리포트가 파일로 저장됨
